...
- [ ] Per-workspace customer OAuth: each workspace owner connects their own account; OAuth state is workspace-scoped; callback validates workspace + stores per-workspace encrypted tokens; account discovery + sync are workspace-scoped; cross-tenant 404; no token leakage across tenants.
- [ ] Customer-facing copy: when platform credentials are absent, customers see "setup pending — ExploringToKnow is finishing the provider's API setup" (NOT "set GOOGLE_ADS_CLIENT_ID…"); no env-var-name lists shown to customers; connect framed as "connect your own account, tokens workspace-scoped and never shared."
- [ ] Docs: PROVIDER_API_AUDIT.md §4b documents the multi-tenant OAuth model; .env.example marks the Google Ads vars as platform-level SaaS credentials customers never provide.
- [ ] Operator go-live prerequisite (one-time, platform): Google Cloud OAuth Web client + redirect URI, Google Ads developer token, prod env values, app recreate — after which every workspace owner self-connects.

## 42. Phase 31 — Google Ads Read Sync v1 (env-gated; live blocked by missing credentials)
**VERIFIED LIVE 2026-06-20 — prod HEAD `0cb27f5`, image `etk-web` (id `sha256:d0a7b156…`) healthy; migrations 25→26; content unchanged (gen 5/art 5/media 45). Google Ads env ABSENT → live OAuth/sync NOT exercised; NO external Google API call occurred (every live path returned 422 before any fetch). Temp-owner E2E (created→checked→deleted, zero residue): pages render (source legend + Connect-Google-Ads prompt, no api-synced section at 0 rows); start → 422 not_configured (env NAMES only, no consent redirect); callback (vault absent) → 422; create connection → not_configured; sync → 422 not_configured (no external call); unauth sync → 401; cross-tenant sync/select → 404; 0 synced rows/accounts/sync-runs; ETK 0 connections. READ-ONLY — no mutate/launch/spend.**
- [ ] `/app/provider-connections/google_ads`: Connect (owner/admin) starts Google OAuth when configured (else disabled with env names); GoogleAdsSyncPanel shows connected account(s), selected account, last sync, last error, "Sync last 30 days" + optional date range. Editor/viewer: status + synced data only, no connect/sync.
- [ ] OAuth: start owner/admin only (editor/viewer cannot), unauth → 401, env-missing → 422 not_configured (env NAMES only); state is signed + workspace-bound; callback validates state/workspace, rejects missing/invalid; token exchange only when env configured; tokens stored encrypted; status → connected on success, error on failure (no secret exposure).
- [ ] Account identity: accessible customers fetched read-only into provider-accounts (workspace-scoped); multi-account supported with a selected account; cross-tenant accounts never appear.
- [ ] Read sync: `/[id]/sync` owner/admin only; creates provider_sync_run with window + status + counts; default last 30 days, max 90; idempotent window replace; partial failures recorded; env-missing/not-connected → 422; cross-tenant → 404.
- [ ] Synced data model: synced_performance_daily rows are source=api_synced/google_ads, workspace-scoped, raw metrics verbatim + derived cost (cost_micros/1e6); conversionValue only as Google-reported; no inferred revenue; manual performance-entries untouched.
- [ ] Performance dashboard: Google Ads api-synced section is clearly labeled and SEPARATE from manual/imported and from internal landing-page views; shown only when synced data exists; honest empty state otherwise; calculated metrics (CTR/CPC/ROAS) handle zero denominators (→ "—").
- [ ] No Google Ads mutate/write endpoint is used (search/listAccessibleCustomers only); no campaign create/launch/budget change; tokens never appear in UI/API/logs; no provider secrets printed/committed.
- [ ] If credentials present (operator): verify live OAuth start→consent→callback→connected, account fetch, manual sync writes api_synced rows, sync-run logs. (NOT verified this deploy — env absent.)
- [ ] Safety: no Meta/TikTok/LinkedIn/Pinterest/other API calls; no AI/image/video/email/billing/social/ad-write/external side effects beyond the intended (env-gated) Google OAuth/read-only calls; public magazine + landing pages + Social Studio + Ads Studio + Manual Performance + gates unchanged; migration additive (down() safe).
...
- [ ] `PROVIDER_API_AUDIT.md` exists with: per-provider audit (Google Ads, Meta, TikTok, LinkedIn, Pinterest + future notes) with **official source URLs** and `⚠ CONFIRM` flags on unverified granular items; an evidence-based **priority table**; **recommended first provider = Google Ads (read sync)** with reasons; normalized **provider-connection architecture** (design only); corrected **API-first roadmap Phase 30→38**; and **hard safety rules**.
- [ ] Data-source labeling principle documented (internal / api-synced / manual / calculated) for future dashboards.
- [ ] Safety rules for future API work documented: no secrets printed/committed; OAuth tokens encrypted at rest; owner/admin-only connections; tenant/workspace-scoped; read-before-write; write starts draft/paused only; no launch/spend without explicit confirmation; no auto-optimization yet; source labels; sync logs + failure handling; kill switch + audit log before publishing.
- [ ] Confirm **no app behavior changed**: public magazine, landing pages + analytics, Brand Kit, Social Studio, Ads Studio, manual Performance, gates, and local-safe email/billing all unchanged; manual performance entry/import still works exactly as in Phase 28.

## 39. Phase 28 — Manual performance import + measurement foundation
**VERIFIED LIVE 2026-06-20 — prod HEAD `1196db8`, image `etk-web` (id `sha256:92fbe151…`) healthy; migrations 23→24; content unchanged (gen 5/art 5/media 45); landing_page_views intact. Temp-owner E2E (created→checked→deleted, zero residue): unauth create 401; bad date → 422; manual entry with foreign relatedProduct nulled; CSV import preview 2 accepted/1 rejected (bad date + reason) + sample, commit created 2 under one importBatchId; overview computed CTR 2%/CVR 5% live; archiving excluded the entry from totals (12,000 not 22,000); foreign PATCH → 404; ETK + other tenant 0 entries. Manual-only — no OAuth/account-connection/external API/sync/fake metrics/AI.**
- [ ] `/app/performance` (owner/admin/editor): Overview (totals + calculated averages + top campaigns/products + landing-page section) · New entry · Import CSV · `/[id]` detail. Viewer → read-only. Sidebar "Performance" under Growth (alongside Analytics).
- [ ] Manual entry: platform/channel/date(+range)/campaign/ad-set/creative; related ad-campaign/ad-creative/landing-page/social-post/product/request/article pickers (workspace-only); metrics impressions/clicks/spend/conversions/revenue/leads/orders/addToCart; currency; notes. Live computed-metrics panel updates as you type.
- [ ] Computed metrics: CTR=clicks/impr, CPC=spend/clicks, CPM=spend/impr×1000, CVR=conv/clicks, CPA=spend/conv, ROAS=rev/spend, Rev/click=rev/clicks. Zero denominators → "—" (never a fake 0). Labelled "calculated from manually entered/imported data". Raw vs calculated kept separate.
- [ ] CSV paste import: expected columns shown; Preview returns accepted/rejected counts + reasons + sample; bad dates rejected with reason; commit creates rows under one importBatchId; landing_page_slug/url mapped to a workspace landing page only if matched; no file upload, no platform sync.
- [ ] Internal landing-page views (Phase 24) surfaced **separately** from manual ad clicks — not blended, not presented as ad clicks.
- [ ] Status flow manual: record / draft / archive / restore. Archived entries excluded from overview totals.
- [ ] Tenant isolation: a workspace only sees its own entries; pickers show only workspace records; no ETK/other-tenant leakage. Cross-tenant tamper (foreign related ids) rejected (stored null); foreign-entry PATCH/action → 404.
- [ ] Role/access: create/edit/import/archive require canWrite (401 unauth, 403 viewer); writes server-scoped (client tenant/workspace ids ignored); dates validated; numbers coerced non-negative.
- [ ] Attribution honesty: copy states manual entries are user-provided, not independently verified, ROAS is calculated from entered spend/revenue, no platform sync yet, no automatic attribution/AI recommendations.
- [ ] Safety: no OAuth/account-connection/ad-API/social-API/external/launch/spend/AI/image/video/email/billing/affiliate-rewrite calls; no fake metrics; public magazine + landing pages + analytics + Social Studio + Ads Studio + gates unchanged; migration additive (down() safe).

## 38. Phase 27 — Ads Studio v1 (manual campaign + creative drafts)
**VERIFIED LIVE 2026-06-20 — prod HEAD `4997a44`, image `etk-web` (id `sha256:2dbea910…`) healthy; migrations 22→23; content unchanged (gen 5/art 5/media 45). Temp-owner E2E (created→checked→deleted, zero residue): unauth create 401; destination data:/javascript: → 422; foreign relatedProduct+relatedSocialPost nulled; creative under campaign OK, foreign-campaign creative → 422; approve → approved_to_export; export returned campaign+creative text + CSV with UTM tracking URL (utm_campaign=spring+sale), mark-exported bumped exportCount; foreign PATCH → 404; campaign DELETE cascaded its creative; ETK + other live tenant had 0 campaigns. Manual + pre-API — no ad accounts/OAuth/launch/spend/AI/external.**
- [ ] `/app/ads` (owner/admin/editor): list + New; `/new` create campaign; `/[id]` campaign editor + inline creative manager. Viewer → read-only. Sidebar "Ads Studio" under Growth.
- [ ] Campaign editor: platform + objective selectors with per-platform helper copy; audience/geography/language/placement notes; budget + schedule **planning notes** (clearly "not real spend"); related product/request/landing-page/social-post pickers (workspace-only; "Use link →" manual prefill); Brand Kit helper.
- [ ] UTM builder: utm_source/medium/campaign/content/term + destination URL → live **final tracking URL** preview (URL-encoded, no shortening/tracking/pixels); copy button. Only builds from a valid http(s) destination.
- [ ] Creative manager (inline): add/edit/status/delete creatives (headline variants, primary text, description, CTA label+URL, format, display path, keywords); "import copy from social post" copies text (not generated); CTA URL http(s)-only.
- [ ] Status flow manual: Save / Mark ready / Approve to export / Archive/Restore (campaign + creative). Approve-to-export is the furthest a record goes — nothing launches/spends/connects an account.
- [ ] Export: per-campaign (and bulk) → copy-friendly text + CSV (platform, objective, audience, budget notes, landing/destination + final tracking URL, headline, primary text, description, CTA, disclosure). "Mark as exported" is explicit + only updates exportCount/exportedAt. Not a launch.
- [ ] Tenant isolation: a workspace only sees its own campaigns/creatives; pickers show only workspace records; no ETK/other-tenant leakage. Cross-tenant tamper (foreign related ids / foreign campaign for a creative) rejected; foreign campaign PATCH/action → 404.
- [ ] Role/access: create/edit/export/delete require canWrite (401 unauth, 403 viewer); writes server-scoped (client tenant/workspace ids ignored); destination + CTA reject javascript:/data:. Campaign delete cascades its creatives.
- [ ] Safety: no ad account connection/OAuth/ad API/launch/budget spend/AI/image/video/email/billing/social-publish/external calls; no fake metrics; public magazine + landing pages + Social Studio + gates unchanged; migration additive (down() safe).
