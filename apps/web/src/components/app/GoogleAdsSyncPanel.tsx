...
type Account = { id: string | number; providerAccountId: string; providerAccountName: string; manager: boolean; selected: boolean };
type Props = {
...
/**
 * Google Ads read-sync controls (Phase 31 / 31A). Owner/admin/super can discover
 * accounts, pick one, and run a READ-ONLY sync. Nothing in Google Ads is changed; no
 * tokens/secrets are shown.
 */
export function GoogleAdsSyncPanel({ connectionId, connected, canManage, accounts, lastSync, connectedAt, lastError }: Props) {
  ...
  async function discover() {
    ...
    if (j?.ok) {
      ...
      if (n === 0) setErr('Connected, but Google returned no accessible accounts. Likely cause: your platform developer token is at Test access (only test accounts work) but a real account was authorized — or this Google login has no Google Ads accounts. Reconnect with a matching account.');
      else { setMsg(`Discovered ${n} account${n === 1 ? '' : 's'}.`); router.refresh(); }
    }
  }
  ...
  async function sync() {
    ...
    if (j?.ok) { setMsg(`Synced ${j.recordsWritten} day-rows (${j.windowStart} → ${j.windowEnd}). Read-only — nothing in Google Ads changed.`); router.refresh(); }
  }
  ...
  return (
    <div style={{ marginTop: 12, borderTop: '1px solid var(--adm-line, #e5e0d6)', paddingTop: 12 }}>
      ...
      <p className="adm-note" style={{ marginBottom: 8 }}>Read-only. Nothing changes in Google Ads · no campaigns are launched · no budgets are changed. Metrics sync from the Google Ads API and are labeled <code>api_synced</code>.</p>
      ...
      {hasAccounts ? (
          ...
          <div className="adm-row"><span className="t">Connected account</span>
            {canManage && accounts.length > 1 ? (
              <select className="adm-select" value={selected ? String(selected.providerAccountId) : ''} disabled={busy} onChange={(e) => selectAccount(e.target.value)}>
                {accounts.map((a) => <option key={a.id} value={String(a.providerAccountId)}>{(a.providerAccountName || `Customer ${a.providerAccountId}`)} · {a.providerAccountId}{a.manager ? ' · manager' : ''}{a.selected ? ' · selected' : ''}</option>)}
              </select>
            ) : (
              <strong>{selected ? `${selected.providerAccountName || `Customer ${selected.providerAccountId}`} · ${selected.providerAccountId}${selected.manager ? ' · manager' : ''}` : '—'}</strong>
            )}
          </div>
          {canManage ? (
        ...
        <div style={{ marginTop: 8 }}>
          <p className="adm-note" style={{ marginBottom: 8 }}><strong>Connected — account discovery hasn’t run yet.</strong> Discover the Google Ads accounts this login can read (read-only).</p>
