...
type Props = {
  provider: string;
  providerName: string;        // short display name (e.g. "Meta Ads", "Google Ads")
  canManage: boolean;
  configured: boolean;
  comingSoon: boolean;
 ...
 */
export function ProviderConnectionControls({ provider, providerName, canManage, configured, comingSoon, connectionId, connectionStatus }: Props) {
  ...
  async function call(url: string, method: string) {
    ...
    try {
      ...
      if (!r.ok || j.ok === false) {
        if (j.code === 'not_configured') setErr(`${providerName} isn’t available to connect yet — ExploringToKnow is finishing ${providerName} API setup.`);
    ...
    <div style={{ marginTop: 10 }}>
      ...
      {!configured ? (
        <p className="adm-note" style={{ marginBottom: 8 }}>
          <strong>Not available yet.</strong> ExploringToKnow is finishing {providerName} API setup. Once that’s done you’ll be
          able to connect your own {providerName} account here with one click. Manual performance import remains available as a fallback.
        </p>
      ) : null}
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button type="button" className="adm-btn" disabled={busy || !configured}
          ...
          {!configured ? 'Connect (setup pending)' : connectionStatus === 'connected' ? 'Reconnect / change account' : `Connect ${providerName}`}
        </button>
