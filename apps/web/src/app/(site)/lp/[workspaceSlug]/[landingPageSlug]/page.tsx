...
import { isSafeHttpUrl, type Section } from '@/lib/landing-constants';
import { SITE_NAME, SITE_URL } from '@/lib/public';
import { LandingViewTracker } from '@/components/site/LandingViewTracker';
...
export default async function PublicLandingPage({ params }: Args) {
  ...
  return (
    <section className="section">
      <LandingViewTracker id={page.id as string | number} />
      <div className="container" style={{ maxWidth: 760 }}>
        ...
        {sections.length ? (
          <div style={{ marginTop: 18, lineHeight: 1.7 }}>
            {sections.map((s, i) => <div key={i}><LpSection s={s} accent={accent} primary={primary} /></div>)}
          </div>
        ) : paras.length ? (
          <div style={{ marginTop: 18, lineHeight: 1.7 }}>
...
/** Render one whitelisted, escaped section. No raw HTML is ever injected. */
function LpSection({ s, accent, primary }: { s: Section; accent: string; primary: string }) {
  const heading = s.heading ? <h2 style={{ color: primary, marginTop: 22, marginBottom: 8 }}>{s.heading}</h2> : null;
  const textBlock = s.text ? s.text.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean).map((p, i) => <p key={i} style={{ marginBottom: 12 }}>{p}</p>) : null;
  if (s.type === 'feature_list' || s.type === 'faq_placeholder') {
    return <div>{heading}<ul>{(s.items || []).map((it, i) => <li key={i} style={{ marginBottom: 6 }}>{it}</li>)}</ul></div>;
  }
  if (s.type === 'pros_cons') {
    return (
      <div>{heading}
        <div className="adm-cols-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div><strong>Pros</strong><ul>{(s.pros || []).map((p, i) => <li key={i}>{p}</li>)}</ul></div>
          <div><strong>Cons</strong><ul>{(s.cons || []).map((c, i) => <li key={i}>{c}</li>)}</ul></div>
        </div>
      </div>
    );
  }
  if (s.type === 'cta_block') {
    const u = isSafeHttpUrl(s.ctaUrl) ? String(s.ctaUrl).trim() : null;
    return u && s.ctaLabel ? <p style={{ marginTop: 16 }}><a href={u} className="btn btn-accent" style={{ background: accent, borderColor: accent }} rel="nofollow sponsored noopener" target="_blank">{s.ctaLabel}</a></p> : null;
  }
  if (s.type === 'disclosure') {
    return s.text ? <p className="meta" style={{ fontSize: 13, color: 'var(--ink-soft, #6b7480)', marginTop: 14 }}>{s.text}</p> : null;
  }
  // text + product_highlight
  return <div>{heading}{textBlock}</div>;
}
