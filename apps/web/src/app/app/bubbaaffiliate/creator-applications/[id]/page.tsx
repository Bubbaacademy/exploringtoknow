...
export default async function CreatorApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Ownership + kind guard — never expose another workspace's or a non-creator doc.
  if (!doc) notFound();
  ...
  if (doc.source !== BUBBA_SOURCES.creator) notFound();

  return <SubmissionDetail doc={doc} kind="creator" editable={canWrite(ws.role)} />;
}
